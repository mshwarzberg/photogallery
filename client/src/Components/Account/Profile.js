...
function Profile() {
  ...
  useEffect(() => {
    axios
      .post("http://localhost:5000/profile", { token: sessionStorage.getItem("token") })
      .then((res) => {
    ...
    axios.post('http://localhost:5000/profile/getinfo', { token: sessionStorage.getItem("token") })
         .then((res) => {
  ...
  return (
    <div className="page">
      ...
      <p>You have {userInfo.numberofimages} images stored, a total of {userInfo.storageused} used, with an average image size of {userInfo.averagesize} , </p>
      <button
        onClick={() => {
          navigate("/gallery");
        }}
